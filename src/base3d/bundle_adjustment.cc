...
double pose_refinement(Eigen::Vector3d& rvec,
  ...
  ceres::LossFunction *loss_function
    = new ceres::CauchyLoss(loss_scale_factor);
...
void _bundle_adjustment_fill_problem(
  ...
  for (auto it=image_ids.begin(); it!=image_ids.end(); ++it) {
    ...
    if (num_residuals > 1) {
      ...
      // TODO: Allow for on-the-fly calibration by adding constraints to
      //       the camera parameters (otherwise they will be changed nearly
      //       "arbitrarily")
      problem.SetParameterBlockConstant(camera_params_data);
    }
...
void _bundle_adjustment_add_pose_constraints(
  ...
  const Eigen::Matrix3d R_C
    = angle_axis_from_rvec(constrain_rvecs.at(image_id)).toRotationMatrix();
  ...
  for (auto it=free_image_ids.begin(); it!=free_image_ids.end(); ++it) {
    ...
    const Eigen::Vector3d& rvec0 = constrain_rvecs.at(image_id);

    ceres::CostFunction* cost_function
      = BARotationConstraintCostFunction::create(constrain_rot_weight, rvec0);
...
double bundle_adjustment(
  ...
  if ((fixed_image_ids.size() * 6 + fixed_x_image_ids.size()) < 7) {
    throw std::invalid_argument("At least parameters should be set as fixed "
  ...
  // deallocated automatically to reduce memory footprint
  {
    ...
    _bundle_adjustment_fill_problem(problem, BA_POSE_FREE,
                                    ...
                                    points2D, points3D, point3D_ids,
                                    point3D_num_points2D,
                                    ordered_point3D_ids, min_track_len,
                                    loss_scale_factor);
    // Fixed images
    ...
    if (constrain_poses) {
      ...
      _bundle_adjustment_add_pose_constraints(problem, feature_manager,
                                              free_image_ids,
                                              fixed_image_ids,
